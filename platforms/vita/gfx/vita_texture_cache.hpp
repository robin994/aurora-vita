...
private:
  ...
  struct Entry{
    ...
    uint64_t key=0,lastUse=0;
    size_t bytes=0;
    bool hasMipmaps=false,cacheable=true;
    uint64_t sourceId=0,paletteSourceId=0;
    size_t sourceBytes=0,paletteBytes=0;
#if !defined(AURORA_VITA_RENDERER_GXM)
    unsigned gl=0;
    ...
    uint8_t explicitMipCount=0,appliedMipCount=0;
#endif
  };
