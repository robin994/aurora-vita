...
elseif (_aurora_sdl3_provider STREQUAL "vendor")
  ...
  if (NOT TARGET SDL3-static AND NOT TARGET SDL3-shared)
    ...
    if (VITA AND AURORA_VITA_SDL3_NATIVE)
      # VitaSDK has no shared-library SDL deployment model. Keep SDL3 as the native
      # platform/input/filesystem layer and let Aurora/vitaGL own all graphics work.
      set(AURORA_SDL3_LINKAGE "static" CACHE STRING "SDL3 linkage type preference" FORCE)
      ...
      set(SDL_GPU OFF CACHE BOOL "Disable SDL GPU API on Vita; Aurora uses vitaGL" FORCE)
      set(SDL_RENDER OFF CACHE BOOL "Disable SDL renderer subsystem on Vita; Aurora/vitaGL owns rendering" FORCE)
      set(SDL_RENDER_GPU OFF CACHE BOOL "Disable SDL GPU renderer on Vita" FORCE)
      set(SDL_HAPTIC OFF CACHE BOOL "Vita input uses the native gamepad driver without a haptic subsystem" FORCE)
      set(SDL_HIDAPI OFF CACHE BOOL "Use SDL3 native Vita joystick driver" FORCE)
      set(SDL_VIRTUAL_JOYSTICK OFF CACHE BOOL "Disable virtual joysticks on Vita" FORCE)
      set(SDL_CAMERA OFF CACHE BOOL "Disable unused Vita camera subsystem" FORCE)
      set(SDL_DIALOG OFF CACHE BOOL "Disable unsupported Vita file dialogs" FORCE)
      set(SDL_TRAY OFF CACHE BOOL "Disable unsupported Vita tray subsystem" FORCE)
      set(SDL_PROCESS OFF CACHE BOOL "Disable unsupported Vita process subsystem" FORCE)
      set(SDL_OFFSCREEN OFF CACHE BOOL "Disable SDL offscreen video driver on Vita" FORCE)
      set(VIDEO_VITA_PIB OFF CACHE BOOL "Aurora Vita uses vitaGL, not PIB" FORCE)
      set(VIDEO_VITA_PVR OFF CACHE BOOL "Aurora Vita uses vitaGL, not PVR" FORCE)
      message(STATUS "aurora: SDL3 Vita native platform profile enabled (vitaGL owns rendering)")
    elseif (AURORA_SDL3_LINKAGE STREQUAL "static")
      set(SDL_STATIC ON CACHE BOOL "Build SDL3 static library" FORCE)
      set(SDL_SHARED OFF CACHE BOOL "Build SDL3 shared library" FORCE)
    else ()
